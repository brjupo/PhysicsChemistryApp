...
function printSubtopics()
{
  ...
  $statement = mysqli_prepare($con, "SELECT id_leccion, nombre, tiempo_examen FROM leccion WHERE id_subtema = ? ORDER BY orden");
  mysqli_stmt_bind_param($statement, "i", $idSubtema);
  mysqli_stmt_execute($statement);

  mysqli_stmt_store_result($statement);
  mysqli_stmt_bind_result($statement, $id_leccion, $nombre, $tiempo_examen);
  ...
  while (mysqli_stmt_fetch($statement)) { //si si existe la leccion
    ...
    $arregloTemas[$i]["tiempo_examen"] = $tiempo_examen;
    $i = $i + 1;
  }
  ...
  for ($i = 0; $i < $tamanho; $i++) {
    ...
    printLections($arregloTemas[$i]["id_leccion"], $arregloTemas[$i]["nombre"], $arregloTemas[$i]["tiempo_examen"]);
  }
}

function printLections($ID_Lection, $LectionName, $examTime)
{
  echo '
    <div class="container">
      <div class="row">
        <div class="input-group col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
          <input type="text" class="form-control" id="' . $ID_Lection . '" value="' . $examTime . '" />
...
function printHead()
{
  ...
  <head>
    ...
    <script src="tiempoExamenLeccion2.js"></script>
  </head>
  ';
}
function printTitle()
...
function printInstructions()
{
  ...
  <div class="container">
    <div class="row">
      <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
        <p>
          - Para cambiar el tiempo de <strong>TODO EL EXAMEN</strong>, ubique la <strong>lección</strong> correspondiente,
