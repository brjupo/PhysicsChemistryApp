...
<body>
    ...
    try {
        ...
        $stringQuery = "SELECT DISTINCT a.nombre, u.mail, g.nombre, pu.tipo FROM asignatura a JOIN grupo g JOIN profesor prof JOIN usuario_prueba u JOIN puntuacion pu ON g.id_asignatura = a.id_asignatura AND g.id_profesor = prof.id_profesor AND prof.id_usuario = u.id_usuario WHERE g.id_grupo = 9 AND pu.tipo = 'SP';";
    ...
    try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $stringQuery = "SELECT t.nombre, s.nombre, l.nombre FROM leccion l JOIN subtema s JOIN tema t ON l.id_subtema = s.id_subtema AND s.id_tema = t.id_tema WHERE t.id_asignatura = (SELECT id_asignatura FROM grupo WHERE id_grupo = 9);";
        $stmt = $conn->query($stringQuery);
        while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            ...
            <table>
                <tbody>
                    ...
                    try {
                        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        $stringQuery = "SELECT a.matricula, SUM(p.puntuacion) AS 'diamantes' FROM puntuacion p JOIN usuario_prueba u JOIN alumno a ON p.id_usuario = u.id_usuario AND u.id_usuario = a.id_usuario WHERE a.id_alumno IN (SELECT id_alumno FROM alumno_grupo WHERE id_grupo = 9) GROUP BY a.matricula ORDER BY matricula ASC;";
                        $stmt = $conn->query($stringQuery);
                        while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
                            //row [0] -> matricula
                            //row [1] -> diamantes
                            ...
                            try {
                                ...
                                $stringQuery2 = "SELECT a.matricula, l.id_leccion, p.puntuacion FROM alumno a JOIN leccion l JOIN puntuacion p ON p.id_leccion = l.id_leccion AND p.id_usuario = a.id_usuario WHERE p.tipo = 'SP' AND a.id_alumno IN (SELECT id_alumno FROM alumno_grupo WHERE id_grupo = 9) ORDER BY a.matricula ASC, l.id_leccion ASC;";
